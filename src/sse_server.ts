...
export function startSSEServer() {
  ...
  function startHttpServer() {
    ...
    httpServer.listen(PORT, HOST, () => {
      ...
      if (configuration.enableHttps) {
        console.log(
          `⚠️  HTTP server running alongside HTTPS - consider disabling HTTP in production`
        );
      }
    });
    return httpServer;
  ...
  function startHttpsServer() {
    if (!configuration.sslKeyPath || !configuration.sslCertPath) {
      ...
      console.log(
        `❌ HTTPS enabled but SSL_KEY_PATH and SSL_CERT_PATH not provided`
      );
      console.log(
    ...
    try {
      ...
      if (!fs.existsSync(configuration.sslCertPath)) {
        throw new Error(
          `SSL certificate file not found: ${configuration.sslCertPath}`
        );
      }
      ...
      httpsServer.listen(HTTPS_PORT, "127.0.0.1", () => {
        ...
        console.log(
          `🔒 ClickUp MCP Server (HTTPS) started on https://127.0.0.1:${HTTPS_PORT}`
        );
        console.log(
          `📡 Streamable HTTPS endpoint: https://127.0.0.1:${HTTPS_PORT}/mcp`
        );
        console.log(
          `🔄 Legacy SSE HTTPS endpoint: https://127.0.0.1:${HTTPS_PORT}/sse`
        );
        console.log(
          `❤️  Health check HTTPS: https://127.0.0.1:${HTTPS_PORT}/health`
        );
      });
      return httpsServer;
    } catch (error) {
      ...
      console.log(
        `❌ Failed to start HTTPS server. Please check the server configuration and logs for details.`
      );
      return null;
    }
  }
  ...
  if (!configuration.enableHttps) {
    console.log(
      `⚠️  HTTPS disabled (set ENABLE_HTTPS=true with SSL certificates to enable)`
    );
  }
